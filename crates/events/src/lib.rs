...
pub trait Exec: Sized {
    ...
    fn drop_events(caller: wasmtime::Caller<'_, Self::Context>, handle: u32) -> Result<(), Trap>;
}

#[derive(Default)]
pub struct ExecTables {
    pub events_table: wit_bindgen_wasmtime::Table<()>,
}
...
pub fn add_to_linker<T: Exec>(linker: &mut wasmtime::Linker<T::Context>) -> anyhow::Result<()> {
    ...
    linker.func_wrap(
        ...
        move |caller: wasmtime::Caller<'_, T::Context>,
              ...
              arg3: i32,
              arg4: i32,
              arg5: i32| { T::events_listen(caller, arg0, arg1, arg2, arg3, arg4, arg5) },
    )?;
    linker.func_wrap(
        "exec",
        "events::exec",
    ...
    linker.func_wrap(
        ...
        move |caller: wasmtime::Caller<'_, T::Context>, handle: u32| T::drop_events(caller, handle),
    )?;
    Ok(())
}
